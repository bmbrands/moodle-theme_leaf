...
class theme_leaf_core_renderer extends core_renderer {
    ...
    protected function render_custom_menu(custom_menu $menu) {
        ...
        //     $myprofilenav = $this->page->navigation->find('myprofile', navigation_node::TYPE_USER);

        //     $editmyprofiletxt = get_string('editmyprofile');
        //     $editmyprofile    = new moodle_url('/user/edit.php', array('id'=>$USER->id, 'course'=>$COURSE->id));
        //     $myprofilenav->add($editmyprofiletxt, $editmyprofile, $editmyprofiletxt);

        //     $myprofilerenderer = $this->page->get_renderer('theme_leaf', 'topmenu');
        //     $myprofiletree     = $myprofilerenderer->topmenu_tree($myprofilenav, get_string('myprofile'));

        //     $culmyprofilemenu = new custom_menu($myprofiletree, current_language());
        //     foreach ($culmyprofilemenu->get_children() as $item) {
        //         $content .= $this->render_custom_menu_item($item, 1);
        //     }
...
/**
 * theme_leaf_topmenu_renderer
 *
 * @package theme
 */
class theme_leaf_topmenu_renderer extends plugin_renderer_base {

    public function topmenu_tree(navigation_node $navigation, $title) {

        //Suppress the menu if there's nothing to show
        if (!$navigation->has_children() || $navigation->children->count() <= 1 ) {
            return '';
        }
    ...
    protected function topmenu_node(navigation_node $node, $navcounter = 0) {
        ...
        foreach ($items as $item) {
            ...
            $action = empty($item->action)                                                   ? $this->page->url
            : (is_object($item->action) && property_exists($item->action, 'scheme')) ? new moodle_url($item->action)
            :                                                                          new moodle_url('#')
            ;

            $content  = $item->text . "|{$action}\n";
            #$content  = $item->text . "|{$action}" . ' | id: ' . var_export(json_encode($item->id), true) . "\n"; //TJGDEBUG
            #$content  = $item->text . "|{$action}" . ' | display: ' . var_export(json_encode($item->display), true) . "\n"; //TJGDEBUG
